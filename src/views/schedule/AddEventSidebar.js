...
import { X, Check, Trash, Info, Upload, XCircle } from 'react-feather'
...
onError: (e) => {
  ...
  const jobParent = (allJobTypes?.team_expertise.filter(element => element.id === selectedEvent?.extendedProps?.job_type_id)[0]) //Get parent value of current job_type  
 ...
 const uploadImage = async (index) => {
  ...
  const handleAddEvent = () => {
    ...
    toast.success(<ToastComponent title='Event Added' color='success' icon={<Check />} />, {
      ...
      toast.error(<ToastComponent title='Error adding event' color='danger' icon={<XCircle />} />, {
        autoClose: 2000,
        hideProgressBar: true,
        closeButton: false
      })
       ...
       <Form
          onSubmit={handleSubmit(data => {
            if (isObjEmpty(errors)) {
              if ((calloutJobCategory?.value !== "Select..." && calloutJobCategory?.value !== null) && (calloutJobType?.value !== "Select..." && calloutJobType?.value !== null)) {
