...
const TabsVerticalLeft = ({ item, refetchClient }) => {
  ...
  const [contract_local, setcontract_local] = useState(item?.contract_reports ? item?.contract_reports : [])
    ...
    if (pdf) {
       ...
       try {
        ...
        const url = await nhost.storage.getPublicUrl({
          ...
          setcontract_local([...contract_local, {id: result_id, client_id: result_client_id, report_upload_date: result_report_upload_date, report_location: result_report_location }])
    ...
    setLoaderButton(false)
     ...
     try {
      if (deleteId?.email) {
        ...
        setcontract_local(contract_local.filter(item => item.id !== deleteId.id))
        setLoading(false)
        setLoaderButton(false)
        return toast.error(
          <ToastComponent title="Report Deleted" color="danger" icon={<Check />} />,
  ...
  const ReportList = ({item}) => (
      <ListGroupItem >
      {item.map(item => (
      <Row key={item.id} className="d-flex align-items-center mt-1">
      <Col sm="1">
      <b>ID:</b> {item?.id}
      </Col>
      <Col>
