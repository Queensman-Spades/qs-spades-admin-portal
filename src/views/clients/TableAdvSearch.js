...
const DataTableAdvSearch = () => {
  ...
  const handleUpdate = async (updatedRow, newPassword, oldPassword) => {
    ...
    updateClient({
      variables: {
        ...
        client_type: updatedRow.client_type
      },
    }).then(() => {
      toast.success(
        <ToastComponent
  ...
  const addClientRecord = () => {
    ...
    setRow({ active: 1, client_type: "Client" });
    setTimeout(() => {
      setModal(!modal);
    }, 200);
  };
    ...
    try {
      await addClient({
        variables: {
          ...
          client_type: newRow?.client_type
        },
      })
  ...
  const createExportObject = (DataTojson) => {
    ...
    for (const keys in DataTojson) {
      objectsToExport.push({
        ...
        email: DataTojson[keys]?.email,
        full_name: DataTojson[keys]?.full_name,
        phone: DataTojson[keys]?.phone,
        account_type: DataTojson[keys]?.account_type,
        address: DataTojson[keys]?.property_owneds?.[0]?.property?.address || null,
        city: DataTojson[keys]?.property_owneds?.[0]?.property?.city || null,
        community: DataTojson[keys]?.property_owneds?.[0]?.property?.community || null,
        country: DataTojson[keys]?.property_owneds?.[0]?.property?.country || null,
        active: DataTojson[keys]?.active,
        contract_start_date: DataTojson[keys]?.contract_start_date ? moment(DataTojson[keys]?.contract_start_date).format('MM/DD/YYYY') : null,
        contract_end_date: DataTojson[keys]?.contract_end_date ? moment(DataTojson[keys]?.contract_end_date).format('MM/DD/YYYY') : null,
        sign_up_time: DataTojson[keys]?.sign_up_time  ? moment(DataTojson[keys]?.sign_up_time).format('MM/DD/YYYY h:mm a') : null,   
        client_type: DataTojson[keys]?.client_type
      })
    }
    ...
    const dataToExport = () => {
      if (
        ...
        searchEmail?.length ||
        searchOrganization?.length ||
        searchOccupation?.length || searchPhone?.length || searchGender?.length
      ) {
        return createExportObject(filteredData)
      } else {
        return createExportObject(data?.client)
      }
    }
