...
const DELETE_BILLS = gql`
mutation deleteBills($_in: [Int!]!) {
  delete_bills(where: {id: {_in: $_in}}) {
    returning {
      id
    }
  }
...
const DataTablesBasic = ({setModal}) => {
  ...
  //** GQL functions
  const [deleteBills, {loading: deleteBillsLoading}] = useMutation(DELETE_BILLS, {
  ...
  const onDelete = async () => {
      try {
        ...
        toast.success(
          <ToastComponent title="Bills Deleted" color="success" icon={<Check />} />,
          {
            autoClose: 2000,
        ...
        console.log(error)
        toast.error(
          <ToastComponent title="Error Deleting Bills" color="danger" icon={<XCircle />} />,
          {
            autoClose: 2000,
            hideProgressBar: true,
            closeButton: false
  ...
  const ExpandedComponent = ({data}) => {
    ...
    return (
      <Card className='pt-2 px-2'>
        ...
        {data?.image_ids?.map((item, index) => {
            if (item) {
              try {
                const publicUrl = item
                return (
                  <div className='mr-2'>
                    <Avatar img={publicUrl} key={index} imgHeight='40' imgWidth='40'  onClick={() => window.open(publicUrl, "_blank")}/>
                  </div>
                )
              } catch (e) {
                console.log(e)
                return (
                  <div className='mr-2'>
                  <Badge key={index}>No Image</Badge>
                  </div>
                )
              }
            } else {
  ...
  return (
      <Fragment>
        <Card className='overflow-hidden'>
          ...
          <CardTitle tag="h4">View and add Bills</CardTitle>
          ...
          <Exportqs InData={createExportObject(bills)}></Exportqs>
          <div className='react-dataTable'>
            <DataTable
              ...
              expandableRows
              expandableRowsComponent={<ExpandedComponent data={bills?.data} />}
          ...
          <CardFooter>
            ...
            {bills?.length > 0 && <Button size="sm" color='secondary' onClick={() => {
      ...
      <div className='theme-modal-danger'>
        ...
        >
          ...
          <ModalBody>
            {deleteBillsLoading ? <div style={{display: "flex", justifyContent: "center"}}><Spinner /></div> : "Are you sure you want to delete?"}
          </ModalBody>
          {(!deleteBillsLoading || bills.length) > 0 && 
          <ModalFooter>
