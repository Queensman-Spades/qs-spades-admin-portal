...
const DataTableAdvSearch = () => {
  ...
  const advSearchColumns = [
    ...
    {
      name: 'Scheduled Date',
      selector: 'schedulers.date_on_calendar',
      sortable: true,
      minWidth: '200px',
      cell: row => moment(row?.schedulers[0]?.date_on_calendar).format("MMMM Do, YYYY") ?? "No Date"
    },
    {
      name: 'Scheduled Time',
      selector: 'schedulers.time_on_calendar',
      sortable: true,
      minWidth: '100px',
      cell: row => moment(new Date(`01/01/2022 ${row?.schedulers[0]?.time_on_calendar}`)).format("h:mm a") ?? "No Time"
    },
    {
      name: 'Created at',
      sortable: true,
      minWidth: '200px',
      cell: row => moment(row?.request_time).format("MM-DD-YY, h:mm a")
    }
  ]
